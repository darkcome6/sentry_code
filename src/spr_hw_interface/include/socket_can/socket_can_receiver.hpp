...
public:
  ...
  /// \throw SocketCanTimeout On timeout
  /// \throw std::runtime_error on other errors
  CanId receive_fd(void* const data,
                   const std::chrono::nanoseconds timeout = std::chrono::nanoseconds::zero()) const;
  /// Receive typed CAN FD data. Slightly less efficient than untyped interface; has extra copy
  /// and branches \tparam Type of data to receive, must be 64 bytes or smaller \param[out] data A
  /// buffer to be written with data bytes. Must be at least 64 bytes in size \param[in] timeout
  /// Maximum duration to wait for data on the file descriptor. Negative
  ///                    durations are treated the same as zero timeout
  /// \return The CanId for the received canfd_frame, with length appropriately populated
  /// \throw SocketCanTimeout On timeout
  /// \throw std::runtime_error If received data would not fit into provided type
                   ...
                   const std::chrono::nanoseconds timeout = std::chrono::nanoseconds::zero()) const
  {
    ...
    if (ret.length() != sizeof(data))
    {
      throw std::runtime_error{ "Received CAN FD data is of size incompatible with provided "
                                "type!" };
    }
    (void)std::memcpy(&data, &data_raw[0U], ret.length());
    return ret;
