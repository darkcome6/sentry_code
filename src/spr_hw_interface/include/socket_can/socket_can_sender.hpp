...
public:
  /// Constructor
  explicit SocketCanSender(const std::string& interface = "can0", const bool enable_fd = false,
  ...
  /// \param[in] timeout Maximum duration to wait for file descriptor to be free for write.
  /// Negative
            ...
            const std::chrono::nanoseconds timeout = std::chrono::nanoseconds::zero()) const
  {
    ...
    // lint -e586 I have to use reinterpret cast because I'm operating on bytes, see below
    // NOLINT
  ...
  /// \throw SocketCanTimeout On timeout
  /// \throw std::runtime_error on other errors
  void send_fd(const void* const data, const std::size_t length,
               const std::chrono::nanoseconds timeout = std::chrono::nanoseconds::zero()) const;
  /// Send raw data with an explicit CAN id
  /// \param[in] data A pointer to the beginning of the data to send
  /// \param[in] timeout Maximum duration to wait for file descriptor to be free for write.
  ...
  /// \throw SocketCanTimeout On timeout
  /// \throw std::runtime_error on other errors
  void send_fd(const void* const data, const std::size_t length, const CanId id,
               const std::chrono::nanoseconds timeout = std::chrono::nanoseconds::zero()) const;
  /// Send typed data with the default id
  /// \tparam Type of data to send, must be 8 bytes or smaller
  /// \param[in] data The data to send
  ...
  /// \throw std::runtime_error on other errors
  template <typename T, typename = std::enable_if_t<!std::is_pointer<T>::value>>
  void send_fd(const T& data, const CanId id,
               const std::chrono::nanoseconds timeout = std::chrono::nanoseconds::zero()) const
  {
